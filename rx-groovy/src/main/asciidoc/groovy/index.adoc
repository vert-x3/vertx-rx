...
transforms an `Observable<Buffer>` in a format the mapper can parse  into an object observable. In order to use it
to parse YAML you can do the following:

[source,groovy]
----
include::unmarhsallYaml.groovy[tags=example]
----
