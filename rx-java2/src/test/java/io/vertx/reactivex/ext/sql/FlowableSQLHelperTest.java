 ...
 */
public class FlowableSQLHelperTest extends VertxTestBase {
  ...
  private void assertTableContainsInitData() throws Exception {
    FlowableSQLHelper.withConnection(client, this::uniqueNames).test()
      .await()
      .assertComplete()
      .assertValueSequence(NAMES.stream().sorted().distinct().collect(toList()));
  }

  private Flowable<String> uniqueNames(SQLConnection conn) {
  ...
  private TestSubscriber<String> inTransaction(Function<SQLConnection, Completable> postInsert) throws Exception {
    return FlowableSQLHelper.inTransaction(client, conn -> {
      return conn.rxExecute(String.format(INSERT_FOLK_SQL, "Georges"))
        ...
        .concatWith(postInsert.apply(conn).toFlowable());
    }).test()
      .await()
