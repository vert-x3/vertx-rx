...
A {@link io.vertx.core.streams.WriteStream}, like a `org.reactivestreams.Subscriber`, consumes items and collaborates with producers to avoid overflows.
...
NOTE: When the `Flowable` terminates successfully, the adapter invokes the {@link io.vertx.core.streams.WriteStream#end()} method.
...
NOTE: If the {@link io.vertx.core.streams.WriteStream} fails, the adapter cancels the `org.reactivestreams.Subscription`.
