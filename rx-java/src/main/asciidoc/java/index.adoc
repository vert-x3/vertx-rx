...
----
compile 'io.vertx:vertx-rx-java:3.5.1-SNAPSHOT'
----
...
The _Rxified_ helper for unmarshalling also supports a custom mapper, so if you want to unmarshal YAML you can
include the `jackson-dataformat-yaml`artifact in your POM and use the following code
...
fileSystem.open("/data.txt", new OpenOptions(), result -> {
 AsyncFile file = result.result();
 Observable<Buffer> observable = file.toObservable();
 observable.lift(io.vertx.rxjava.core.RxHelper.unmarshaller(MyPojo.class, mapper)).subscribe(
     mypojo -> {
       // Process the object
     }
 );
