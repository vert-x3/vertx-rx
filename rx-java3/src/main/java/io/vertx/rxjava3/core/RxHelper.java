...
public class RxHelper {
  ...
  public static Scheduler blockingScheduler(WorkerExecutor executor) {
    return io.vertx.rxjava3.RxHelper.blockingScheduler(executor.getDelegate());
  }
  ...
  public static Scheduler blockingScheduler(Vertx vertx, boolean ordered) {
    return io.vertx.rxjava3.RxHelper.blockingScheduler(vertx.getDelegate(), ordered);
  }
  ...
  public static Single<String> deployVerticle(Vertx vertx, Verticle verticle, DeploymentOptions options) {
    return RxJavaPlugins.onAssembly(AsyncResultSingle.toSingle(handler -> vertx.getDelegate().deployVerticle(verticle, options).onComplete(handler)));
  }
  ...
  public static Scheduler blockingScheduler(Vertx vertx) {
    return io.vertx.rxjava3.RxHelper.blockingScheduler(vertx.getDelegate());
  }
  ...
  public static Scheduler scheduler(Vertx vertx) {
    return io.vertx.rxjava3.RxHelper.scheduler(vertx.getDelegate());
  }
